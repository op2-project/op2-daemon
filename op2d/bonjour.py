...
from op2d import __version__ as op2d_version
...
class BonjourServices(object):
    ...
    def _build_txtdata(self, transport):
        ip_address = host.default_ip
        ...
        contact_uri = SIPURI(host=ip_address, port=port)
        contact_uri.parameters.update(parameters)
        instance_id = str(uuid.UUID(SIPSimpleSettings().instance_id))
        txtdata = dict(txtvers=1, name='OP2 Phone', version=op2d_version, contact='<%s>' % str(contact_uri), instance_id=instance_id)
        return contact_uri, txtdata
